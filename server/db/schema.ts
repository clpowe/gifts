...
export const birthday = sqliteTable("birthday", {
  ...
  includeYear: integer("include_year", { mode: "boolean" })
    .notNull()
    .default(true),
...
export const calendarSync = sqliteTable("calendar_sync", {
  id: text("id").primaryKey(),
  birthdayId: text("birthday_id")
    .notNull()
    .references(() => birthday.id, { onDelete: "cascade" }),
  userId: text("user_id")
    .notNull()
    .references(() => user.id, { onDelete: "cascade" }),
...
// Birthday relations
export const birthdayRelations = relations(birthday, ({ one, many }) => ({
  organization: one(organization, {
    fields: [birthday.orgId],
    references: [organization.id],
  }),
  createdBy: one(user, {
    fields: [birthday.createdBy],
    references: [user.id],
  }),
  calendarSyncs: many(calendarSync),
}));

// CalendarSync relations
export const calendarSyncRelations = relations(calendarSync, ({ one }) => ({
  birthday: one(birthday, {
    fields: [calendarSync.birthdayId],
    references: [birthday.id],
  }),
  user: one(user, {
